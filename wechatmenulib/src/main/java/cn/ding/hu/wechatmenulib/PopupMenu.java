...
public class PopupMenu extends PopupWindow {
    ...
    public PopupMenu(Context context, View view) {
        ...
        this.view = view;
        VERTICAL_OFFSET = dip2px(context, 10);
        DEFAULT_MENU_WIDTH = dip2px(context, 180);
        screenPoint = getScreenMetrics(context);
    ...
    public void items(int itemWidth, String... items) {
        ...
        for (int i = 0; i < items.length; i++) {
            PopupMenuItem menuModel = new PopupMenuItem();
            menuModel.setItem(items[i]);
            PopupMenuItemList.add(menuModel);
        }
        generateLayout(itemWidth);
    }

    public <T extends PopupMenuItem> void items(List<T> itemList) {
        PopupMenuItemList.clear();
        PopupMenuItemList.addAll(itemList);
        generateLayout(DEFAULT_MENU_WIDTH);
    }

    public <T extends PopupMenuItem> void items(List<T> itemList, int itemWidth) {
        PopupMenuItemList.clear();
        PopupMenuItemList.addAll(itemList);
        generateLayout(itemWidth);
    }
    ...
    private void generateLayout(int itemWidth) {
        ...
        for (int i = 0; i < PopupMenuItemList.size(); i++) {
            ...
            PopupMenuItem menuModel = PopupMenuItemList.get(i);
            if (menuModel.getItemResId() != View.NO_ID) {
                ...
                textView.setCompoundDrawablePadding(dip2px(context, 12));
    ...
    private void readItem(AttributeSet attrs) {
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.PopupMenuItem);
        CharSequence itemTitle = a.getText(R.styleable.PopupMenuItem_menu_title);
        int itemIconResId = a.getResourceId(R.styleable.PopupMenuItem_icon, View.NO_ID);
        PopupMenuItem menu = new PopupMenuItem();
        menu.setItem(String.valueOf(itemTitle));
        if (itemIconResId != View.NO_ID) {
            menu.setItemResId(itemIconResId);
        }
        PopupMenuItemList.add(menu);
        a.recycle();
    }
    ...
    private void readGroup(AttributeSet attrs) {
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.PopupMenuItem);
        CharSequence itemTitle = a.getText(R.styleable.PopupMenuItem_menu_title);
//		int itemIconResId = a.getResourceId(R.styleable.PopupMenuItem_icon, 0);
//		PopupMenuItemList.add(String.valueOf(itemTitle));
        a.recycle();
    }
